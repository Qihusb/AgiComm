...
| 项                 | 值                                        |
| ----------------- | ---------------------------------------- |
| **URL**           | `http://localhost:8001/simulate/inquiry` |
| **Method**        | `POST`                                   |
...
| 参数名          | 类型            | 必选  | 范围        | 说明                            |
| ------------ | ------------- | --- | --------- | ----------------------------- |
| `event_text` | String        | 是   | 1–1000 字符 | 科技/外交事件描述；不可为空或纯空白；支持中英文、特殊字符 |
| `media_ids`  | Array[String] | 否   | 1–15 个 ID | 指定媒体 ID 列表（可选）；若不指定，则不可运行     |
...
```javascript
const response = await fetch("http://localhost:8001/simulate/inquiry", {
  method: "POST",
...
Navbar 组件每 3 分钟自动检查一次后端连接，用户也可点击"重试"按钮手动检查，设置的时间比较长，可以自行在 Navbar.vue 中的 onMounted() 更改。
...
```bash
curl -s -X POST "http://localhost:8001/simulate/inquiry" \
